...
$fcolor = isset($_POST["fcolor"]) ? $_POST["fcolor"] : 0;
$fsize = isset($_POST["fsize"]) ? $_POST["fsize"] : 0;
$ftime = isset($_POST["ftime"]) ? $_POST["ftime"] : 0;
$fuser = isset($_POST["fuser"]) ? $_POST["fuser"] : 0;
$famount = isset($_POST["famount"]) ? $_POST["famount"] : 0;
$flink = isset($_POST["flink"]) ? $_POST["flink"] : 0;
...
switch ($faction) {
    case "list" : {
        ...
        if ($fcount > 0 && $fcurrent > 0) {
            $sql = "select count(*) as t from stock";
            $rs = $conn->query($sql);
            if ($rs) {
                $row= $rs->fetch_assoc();
                $total = $row["t"];
